this.xxhalforcaa_skill <- this.inherit("scripts/skills/skill", {
	m = {
		IsSpent = false
	},
	function create()
	{
		this.m.ID = "actives.xxhalforcaa_skill";
		this.m.Name = "Blood Lust";
		this.m.Description = "Increases combat power with blood lust. Can be used on self or allies.";
	...
	function getTooltip()
	{
		local ret = [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+20%[/color] Melee Attack Damage, \n[color=" + this.Const.UI.Color.PositiveValue + "]+20[/color] Melee Hit Chance, \nLose [color=" + this.Const.UI.Color.NegativeValue + "]10[/color] HP"
			},
			{
				id = 6,
				type = "text",
	...
	function onUse( _user, _targetTile )
	{
		...
		_targetTile.getEntity().getSkills().add(this.new("scripts/skills/effects/xxhalforcaa_effect"));
		return true;
	}
