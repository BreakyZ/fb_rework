this.xxmob_wokousamurai <- this.inherit("scripts/entity/tactical/human", {
	...
	function onInit()
	{
		...
		b.RangedDefense += 5;
		b.Hitpoints += this.Math.floor(this.World.getTime().Days * 0.3) + 100;
		b.Bravery += 50;
		b.Stamina += 50;
		b.Initiative += 20;
		b.Vision += 2;
	...
	function onDeath( _killer, _skill, _tile, _fatalityType )
	{
		...
		if (_killer == null || _killer.getFaction() == this.Const.Faction.Player || _killer.getFaction() == this.Const.Faction.PlayerAnimals)
		{
			...
			{
				loot = this.new("scripts/items/helmets/xxhat_samurai");
				loot.drop(_tile);
			}
			if (this.Math.rand(1, 100) <= 5)
			{
				loot = this.new("scripts/items/armor/xxsamuraiarmor");
				loot.drop(_tile);
			}
