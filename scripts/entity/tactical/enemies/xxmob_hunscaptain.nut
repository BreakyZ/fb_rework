this.xxmob_hunscaptain <- this.inherit("scripts/entity/tactical/human", {
	...
	function onTurnEnd()
	{
		...
		{
			if (this.actor.getTile().hasZoneOfControlOtherThan(this.actor.getAlliedFactions()) || this.Time.getRound() >= 3)
			{
				if (this.actor.getSkills().hasSkill("effects.disarmed"))
				{
					this.actor.getSkills().removeByID("effects.disarmed");
				}
				if (this.actor.getSkills().hasSkill("actives.ai_damaged_move"))
				{
					this.actor.getSkills().removeByID("actives.ai_damaged_move");
				}
				local b = this.m.BaseProperties;
				b.MeleeSkill += 3;
				b.MeleeDefense += 3;
